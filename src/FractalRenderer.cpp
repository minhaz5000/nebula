...
FractalRenderer::FractalRenderer() 
	: enableGUI(true), pauseRendering(false), timeScale(1.0), fractalCenter(0.0f, 0.0f),
	 fractalScale(1.0f), fractalConstant(0.0f, 0.0f), iterations(100),
...
void FractalRenderer::renderFractal() {
	...
	auto guiLoop = [&]() -> bool {
		...
		if (ImGui::BeginCombo("Fractal Type", renderer->getCurrentShaderName().c_str())) {
            if (ImGui::Selectable("Mandelbrot", renderer->getCurrentShaderName() == "Mandelbrot")) {
	...
	auto windowLoop = [&]() -> bool {
		...
		renderer->getCurrentShader()->setUniformVec2("u_resolution", glm::vec2{window->getWindowWidth(), window->getWindowHeight()});
